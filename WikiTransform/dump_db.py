...
class DumpDB:
    def __init__(self, db_file: str):
        ...
        self._env = lmdb.open(db_file, readonly=True,
                              subdir=False, lock=False, max_dbs=3)
    ...
    def resolve_redirect(self, title: str, max_steps: int = 10) -> str:
        ...
        logger.warn(
            f"Max steps ({max_steps}) exceeded when resolving redirect: {title}")
        return cur_title
    ...
    def get_mutipage_paragraphs(self, keys: List[str]) -> List[List[Paragraph]]:
        ...
        return [self._deserialize_paragraphs(value[1]) for value in values]
...
def _parse_page(page: WikiPage, preprocess_func: Optional[Callable[[str], str]]):
    ...
    if media_re is None:
        media_prefixes = "|".join(
            ["File", "Image", "Media"] + MEDIA_ALIASES.get(page.language, []))
    ...
    if category_re is None:
        category_prefixes = "|".join(
            ["Category"] + CATEGORY_ALIASES.get(page.language, []))
        category_re = re.compile(
            f"^(?:{category_prefixes}):", flags=re.IGNORECASE)
    ...
    def parse_node(node: mwparserfromhell.nodes.Node) -> list[str | WikiLinkTuple]:
        if isinstance(node, mwparserfromhell.nodes.Wikilink):
            # ignore the link if it points to a media file
            if media_re.match(str(node.title)):
                return []
        ...
        for node in section.ifilter(recursive=False):
            for str_or_wiki_link in parse_node(node):
                if isinstance(str_or_wiki_link, WikiLinkTuple):
                    ...
                    cur_links.append(
                        (str_or_wiki_link.title, str_or_wiki_link.text, start, end))
